...
class single_input_degdelaysets:
	...
	def make_degdelaysets(self,deg,delay):
		...
		if len(degsets)>1:
			...
			degsets = np.unique(np.array(degsets, dtype=object))
		# print('degsets',degsets)
		...
		if len(degsets)>1:
			degdelaysets = np.unique(np.array(degdelaysets, dtype=object)).tolist()
		...
		print(degdelaysets, degdelaysets_shuffle, threshold_index)
		np.savez("%s/%d_%d.npz"%(self.ddsetdir,deg,delay),degdelaysets=np.array(degdelaysets, dtype=object),
														  degdelaysets_shuffle=np.array(degdelaysets_shuffle, dtype=object),
														  threshold_index=np.array(threshold_index))
