...
class Evolution_MPS:
    def __init__(self, rho, parameter_size, input_size, d_bond, std, device, input_scale):
        self.rho = rho
        ...
        self.iota = input_scale

    def _sigmoid(self, x):
        return np.exp(np.minimum(x, 0)) / (1 + np.exp(- np.abs(x)))

    def __call__(self, x, zeta):
        x_tmp = torch.Tensor(x).reshape(1, self.parameter_size, 1)
